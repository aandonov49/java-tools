...
public class MergeJson {
  ...
  private static DhcpdType dhcp;
  private static FreeradiusauthorizedmacsType radius = factory.createFreeradiusauthorizedmacsType();
  ...
  public static void main(String[] args) throws IOException, JAXBException, InterruptedException {
    ...
    JAXBContext context = JAXBContext.newInstance(PfsenseType.class);
    Marshaller mar = context.createMarshaller();
    Unmarshaller umar = context.createUnmarshaller();
    JAXBElement object = (JAXBElement) umar.unmarshal(new FileInputStream(file));
    PfsenseType pfsenseCof1 = (PfsenseType) object.getValue();
    dhcp = pfsenseCof1.getDhcpd();
                  ...
                  element -> {
                    if (((JAXBElement<?>) element).getName().getLocalPart().equals("name")) {
                      return (((JAXBElement<?>) element).getValue().equals("ad4u-bg.com")
                          || ((JAXBElement<?>) element).getValue().equals("0.1.10.in-addr.arpa"));
                    }
                    return false;
                  })
